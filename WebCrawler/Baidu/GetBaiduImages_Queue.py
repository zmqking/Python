...
class Baidu():
    def __init__(self):
        self.url = 'https://image.baidu.com/search/acjson?tn=resultjson_com&logid=9442100676687252916&ipn=rj&ct=201326592&is=&fp=result&fr=&word=张靓颖&cg=star&queryWord=张靓颖&cl=2&lm=-1&ie=utf-8&oe=utf-8&adpicid=&st=-1&z=&ic=&hd=&latest=&copyright=&s=&se=&tab=&width=&height=&face=1&istype=2&qc=&nc=1&expermode=&nojc=&isAsync=&pn={}&rn=60&gsm=1e&1683115771526='
        self.heads = {
    ...
    async def getJsonData(self):
        async with aiohttp.ClientSession()as client:
            for i in range(2):
                response = await client.get(self.url.format(i), headers=self.heads)
                dict = await response.json()
                if bool(dict):
                    for item in dict['data']:
                        if len(item) > 0 and item['thumbURL'] != None and item['thumbURL'] != '':
                            self.json_queue.put(item['thumbURL'])
